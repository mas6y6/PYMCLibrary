...
class Player:
    def __init__(self,_raw_data: str, _conn):
        self._raw = _raw_data
        self._raw_split = self._raw["return"].split("*")
        self._conn = _conn
    ...
    def update(self) -> None:
        uid = self._conn._send(f"player*{self.uuid}*getplayer")
        self._raw = self._conn._recv(uid)
        self._raw_split = self._raw.split("*")
        self.username = self._raw_split[0]
        self.displayname = self._raw_split[1]
        self.customname = self._raw_split[2]
        if self._raw_split[4] == Gamemode.SURVIVAL:
    ...
    def getlocation(self) -> Location:
        id = self._conn._send(f"player*{self.uuid}*getloc")
        data = self._conn._recv(id)
        if data["status"] == "error":
            raise JavaException(f"An error occurred {data["return"].split("*")[1]}")
        return Location(data,self)
    
    def giveitem(self,itemname: str,amount: int) -> None:
        id = self._conn._send(f"player*{self.uuid}*giveitem*{itemname}*{amount}")
        data = self._conn._recv(id)
        if data["status"] == "error":
            raise JavaException(f"An error occurred {data["return"].split("*")[1]}")
    
    def removeitem(self,itemname: str,amount: int) -> None:
        id = self._conn._send(f"player*{self.uuid}*removeitem*{itemname}*{amount}")
        data = self._conn._recv(id)
        if data["status"] == "error":
            raise JavaException(f"An error occurred {data["return"].split("*")[1]}")
    
    def sendmessage(self,message: str) -> None:
        id = self._conn._send(f"player*{self.uuid}*sendmessage*{message}")
        data = self._conn._recv(id)
        if data["status"] == "error":
            raise JavaException(f"An error occurred {data["return"].split("*")[1]}")
    
    def sendtitle(self,title: str,subtitle: str,fadein=10,stay=70,fadeout=20) -> None:
        id = self._conn._send(f"player*{self.uuid}*sendtitle*{title}*{subtitle}*{fadein}*{stay}*{fadeout}")
        data = self._conn._recv(id)
        if data["status"] == "error":
            raise JavaException(f"An error occurred {data["return"].split("*")[1]}")
    
    def sendactionbar(self,message: str) -> None:
        id = self._conn._send(f"player*{self.uuid}*sendactionbar*{message}")
        data = self._conn._recv(id)
        if data["status"] == "error":
            raise JavaException(f"An error occurred {data["return"].split("*")[1]}")
    
    def addeffect(self,potion: str, duration: int, amplifier: int) -> None:
        id = self._conn._send("player*{self.uuid}*addeffect*{potion}*{duration}*{amplifier}")
        data = self._conn._recv(id)
        if data["status"] == "error":
            raise JavaException(f"An error occurred {data["return"].split("*")[1]}")
        elif data["return"].split("*")[1] == "potionnotfound":
            raise BukkitError("Potion Not Found")
    
    def removeeffect(self,potion: str) -> None:
        id = self._conn._send(f"player*{self.uuid}*removeeffect*{potion}")
        data = self._conn._recv(id)
        if data["status"] == "error":
            raise JavaException(f"An error occurred {data["return"].split("*")[1]}")
