...
class World:
    ...
    def setweather(self,weather: Weather):
        if weather == Weather.CLEAR:
            self._conn._send(f"world*{self.world_name}*setweather*false*false")
        elif weather == Weather.RAIN:
            self._conn._send(f"world*{self.world_name}*setweather*true*false")
        elif weather == Weather.THUNDER:
            self._conn._send(f"world*{self.world_name}*setweather*true*true")
    
    def update(self) -> None:
        uid = self._conn._send(f"world*{self.world_name}*getworld")
